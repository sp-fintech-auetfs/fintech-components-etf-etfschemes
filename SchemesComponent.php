...
class SchemesComponent extends BaseComponent
{
    ...
    public function viewAction()
    {
        ...
        if (isset($this->getData()['timeline'])) {
            try {
                ...
                if ($this->view->timeline !== $this->view->today) {
                    $postUrl = 'etf/schemes/view/q/timeline/' . $this->view->timeline;
                } else {
        ...
        if (isset($this->getData()['id'])) {
            if ($this->getData()['id'] != 0) {
                ...
                if (isset($this->getData()['custom']) && $this->getData()['custom'] == true) {
                    ...
                    $customArr[] = 0;
                    if (isset($scheme['navs']['navs'])) {
                        $this->view->customNavStartDate = $this->helper->first($scheme['navs']['navs'])['date'];
                        $this->view->customNavEndDate = $this->helper->last($scheme['navs']['navs'])['date'];

                        foreach ($scheme['navs']['navs'] as $customNavs) {
                    ...
                    $patternsPackage = $this->usePackage(EtfToolsPatterns::class);
                    ...
                    $this->view->schemeNavChunks = [];

                    if ($scheme['navs_chunks']) {
                    ...
                    $this->view->rollingPeriods = [];

                    if ($scheme['rolling_returns']) {
                ...
                $this->getProcessedSchemeNavChunksAction($scheme, $timeline);
        ...
        $this->generateDTContent(
            ...
            columnsForTable : ['name', 'symbol', 'year_rr', 'two_year_rr', 'three_year_rr', 'five_year_rr', 'seven_year_rr', 'ten_year_rr', 'fifteen_year_rr', 'category_id', 'day_cagr', 'day_trajectory', 'amc_id', 'start_date', 'navs_last_updated'],
            columnsForFilter : ['name', 'symbol', 'year_rr', 'two_year_rr', 'three_year_rr', 'five_year_rr', 'seven_year_rr', 'ten_year_rr', 'fifteen_year_rr', 'category_id', 'day_cagr', 'day_trajectory', 'amc_id', 'start_date', 'navs_last_updated'],
    ...
    public function getProcessedSchemeNavChunksAction(&$scheme, $timeline = false)
    {
        ...
        foreach (['week', 'month', 'threeMonth', 'sixMonth', 'year', 'threeYear', 'fiveYear', 'tenYear', 'fifteenYear', 'twentyYear', 'twentyFiveYear', 'thirtyYear', 'all'] as $time) {
            if ($time === 'week') {
                if (!isset($chunks[$time])) {
                    ...
                    $weekData = $this->schemesPackage->getSchemeNavChunks(['scheme_id' => $scheme['id'], 'chunk_size' => 'week'], false, $timeline);
    ...
    public function toggleWatchlistAction()
    {
        $this->requestIsPost();

        $this->schemesPackage->toggleWatchlist($this->postData()['scheme_id']);
